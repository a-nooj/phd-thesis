...
\cref{ch:prior} organized existing approaches to dynamics-constrained motion generation around the question of \emph{where} knowledge of admissible motion should reside in a robotic system and \emph{what} computational form it should take.
This chapter gives the first answer: feasibility is evaluated entirely \emph{online}, inside the planning loop, by rolling out a physics simulation as the forward model.
Placing constraint knowledge in simulation lets a sampling-based planner discover impulsive, non-prehensile behavior---poking---that expands what a manipulator can do beyond geometric pick-and-place, at the cost of paying the full dynamic-feasibility computation anew on every planning query.
...
\subsection{Simulation Model for Poking}
...
\subsection{Motion Planning for Poking}
...
\caption[Simulation task times and success rates across planners.]{Task times [\textsl{mean (stddev)}] for various planning algorithms in simulation are presented. Success rates are aggregated across all 6 scenarios. Overall, poke planning is faster than push planning and leads to higher success rate than pushing or grasping. Low-impulse (LI) poke planners and the push planner are unsuccessful in S5 and S6 due to action path collisions (S5) or goal region being outside the robot's reachable workspace (S6). The robot is unable to pick or place object in S4 and S6 due to limitations in robot mechanics.}
...
\caption[Number of executed actions per planner in simulation.]{Number of executed actions in the planned path for various algorithms in simulation are presented here. Both \textsl{PokeRRT*} and \textsl{Low-Impulse PokeRRT*} lead to fewer actions than their RRT-based counterparts, thus exploiting the large displacement property inherent to poking manipulation.}
...
\caption[Real-world task performance across planners.]{Real-world task times, number of actions in executed path, and success rates are presented in this table. Results are averaged across 10 trials. Pushing has the highest success rate and fastest task times. PokeRRT* generates plans with the fewest number of actions.}
...
\begin{figure}
  ...
  \caption[Two-robot collaborative poking task in non-overlapping workspace (S6).]{Two robots with non-overlapping reachable regions are shown (S6). Robot A (left) applies 2 pokes to manipulate the object to Robot B's (right) workspace (a-d). Robot B then grasps the object and places it in a bin that is not reachable by Robot A (e-f).}\label{poke:fig:vignette}
\end{figure}
...
\vspace{4pt}
  ...
  \caption[Real-world task time breakdown by planning and execution phase.]{A breakdown of task time as the sum of initial planning, replanning, and execution times is presented for various planners in the real-world. A single bar indicates total task time. Long red bars indicate cases where tasks cannot be solved. Overall, poke planning demonstrates lower execution and replanning times than push planning.}\label{poke:fig:plot}
\end{figure}
...
Because feasibility here is established by rolling out a simulation inside the planning loop, the cost of dynamic reasoning is paid in full each time the robot plans, and the resulting motion structure is discarded once a plan is found.
\cref{ch:constrained} first carries this online, simulation-in-the-loop paradigm into settings where the robot's embodiment or its payload imposes additional dynamic constraints.
\cref{ch:multiquery} then asks whether the repeated cost of dynamic feasibility can instead be \emph{amortized}---computed once and reused across many related queries rather than rediscovered from scratch on every plan.
