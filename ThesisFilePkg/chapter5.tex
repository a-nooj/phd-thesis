...
The planners of \cref{ch:poking,ch:constrained} evaluate dynamic feasibility online, rebuilding their search structure from scratch for every new start and goal and paying the full cost of simulation and collision checking on each query.
This chapter introduces the first \emph{amortized} representation in this thesis: the dynamic feasibility of a robot's motions is precomputed once into a reusable bundle of edges and shared across many related planning queries.
By deferring---and largely reusing---propagation and collision checking through lazy evaluation, this shifts constraint knowledge upstream of any single query, reducing per-query planning time while preserving the kinodynamic feasibility guarantees that an online planner provides.
...
\subsection{Edge Bundle Construction with the Robot Dynamics Model}
...
We benchmark \textsl{LazyBoE} against a number of kinodynamic planners with forward propagation, namely RRT \cite{lavalle2001randomized}, SST \cite{li2016asymptotically}, and BoE \cite{shome2021asymptotically}. We test three variants of SST based on different selection-to-pruning radius ($0.5$x, $1$x, and $2$x).
...
\begin{figure}
  ...
  \caption[Final solution cost comparison across planners.]{The final solution cost, i.e., the solution with the lowest cost, for our planner is comparable to the BoE planner. Asterisks indicate significance levels.}\label{lazy:fig:eval-cost}
\end{figure}

\begin{figure}
  \centering
    \includegraphics[width=.9\linewidth]{../lazy/assets/success_rate.png}
  \caption[Success rate comparison across planners.]{Our algorithm has a higher success rate compared to the baselines owing to the exploration of a greater number of potential solutions (\cref{lazy:fig:eval-num_solutions}).}\label{lazy:fig:eval-success}
\end{figure}

\begin{figure}
  \centering
    \includegraphics[width=.9\linewidth]{../lazy/assets/num_solutions.png}
  \caption[Number of solutions explored per planner.]{Our planner is able to explore a larger number of solutions than the baseline methods. RRT terminates after finding the first solution, but the mean number of solutions is less than $1$ because RRT does not succeed in finding a solution for every planning problem \cref{lazy:fig:eval-success}. Asterisks indicate the significance levels.}\label{lazy:fig:eval-num_solutions}
\end{figure}
...
Amortization here takes the form of an \emph{explicit} precomputed structure: the edge bundle stores dynamically feasible motions directly, and it need only be re-bundled when the collision geometry or dynamics model changes---a one-time cost paid up front and recovered over subsequent queries.
This explicit representation keeps feasibility inspectable and preserves the asymptotic-optimality guarantees of the underlying planner, but it still stores motion structure rather than the conditions that make motion feasible.
\cref{ch:generation,ch:payload} pursue a more aggressive form of amortization, absorbing dynamic feasibility into the parameters of a learned generative model so that feasible trajectories can be sampled directly, without storing or searching an explicit structure at all.
