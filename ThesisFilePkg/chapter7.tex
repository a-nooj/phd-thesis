...
The edge bundles of \cref{ch:multiquery} amortize dynamic feasibility by precomputing and storing an explicit structure of feasible motions.
\cref{ch:generation} reframed motion generation as conditional sampling, setting up a more aggressive alternative: rather than storing feasible motions, a generative model can absorb the \emph{conditions} of feasibility into its parameters during training.
This chapter realizes that idea for a concrete and demanding capability---manipulating payloads well beyond a robot's nominal rating---by conditioning a trajectory diffusion model on a single high-level object property, payload mass, so that dynamically feasible motions can be sampled in constant time without any runtime constraint checking.
...
\begin{figure}
  ...
  \caption[Diffusion model for super-nominal payload trajectory generation.]{The diffusion model presented in this work learns to generate dynamically feasible trajectories directly in joint angle, velocity, and acceleration space, enabling super-nominal payload manipulation.}\label{payload:fig:firstpg}
\end{figure}
...
\begin{wrapfigure}{l}{0.48\textwidth}
  ...
  \caption[1D UNet architecture conditioned on payload embeddings.]{Our model conditions a 1D UNet denoising architecture \cite{chi2023diffusionpolicy} on various payload embeddings.}
...
\begin{wrapfigure}{r}{0.6\textwidth}
  ...
  \caption[Aggregate success rate by payload and conditioning method.]{The aggregate success rate metric shows one-hot diffusion closely matching the underlying training data distribution.}\label{payload:fig:bar}
\end{wrapfigure}
...
\begin{figure}
  ...
  \caption[Planning time and success rate across payload conditions.]{Comparative analysis of trajectory planning methods for payloads of $3$kg, $6$kg, and $9$kg. DDIM (One-Hot) significantly outperforms a variety of baseline methods in both planning time to first solution and success rate, generating dynamically feasible trajectories in constant time across all payload conditions.}\label{payload:fig:eval}
\end{figure}
...
\begin{figure}
  ...
  \caption[Real-world DEFT evaluation: erasing and drawer tasks.]{Left (Erasing): The robot grasps an eraser from the table and performs back-and-forth sweeps to remove text. Right (Drawer): The robot opens a drawer, pushes an object to a graspable pose, places it inside, and closes the drawer.}
  \label{fig:deft:real}
\end{figure}
...
Together, these two instantiations motivate the question taken up in \cref{ch:seeding}: whether the same conditioning principle can be generalized from a single governing property to arbitrary kinematic and dynamic constraints, turning constraint-aware generation into a task-agnostic seeding strategy for the downstream trajectory optimization that closes this thesis.
