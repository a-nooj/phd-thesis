...
% The result is highly capable systems that enable production-scale throughput and human safety within their operational envelopes.
...
However, the physical constraints that govern feasible behavior are distributed across modules and encoded in the forms required by the chosen pipeline: collision models, grasp metrics, reachability checks, controller limits, safety margins, and task-specific heuristics.
...
In this case, however, constraint knowledge is often represented implicitly in the data distribution and the learned model parameters.
The system may produce behavior that respects the constraints present during training, but the physical assumptions under which that behavior remains admissible are difficult to inspect, modify, or validate independently.
...
% And most importantly, in both paradigms, dynamics are typically treated as secondary: classical pipelines approximate them through simplified analytical models, while learned policies absorb them implicitly through demonstrated behavior rather than representing them as explicit constraints \cite{yin2021modeling}.
...
% In both cases, the representation of constraint knowledge---what physical conditions govern feasible behavior and where that knowledge is expressed in the system---is treated as secondary to the choice of algorithm or policy architecture.

% Despite their differences, engineered pipelines and learned policies for robotic manipulation are typically organized around the motion generation algorithm: the planner, optimizer, controller, or policy that produces behavior under a specified set of kinematic and dynamic constraints.
% Under this organization, constraints define the feasible set, while methods are compared by how effectively they search, optimize, approximate, or sample within that set.
% This organization is useful, but it leaves implicit a second, equally consequential design decision: a physical constraint must first be represented computationally before it can shape motion generation.
...
From this perspective, progress in robot manipulation depends not only on better algorithms for searching, optimizing, or learning behavior, but also on better ways of expressing the constraints that define what behavior is physically possible.
This motivates the central question of this thesis: where should knowledge of admissible motion reside in a robotic system, what computational form should it take, and how does that choice shape downstream motion generation?
By presenting multiple answers to these questions, this thesis makes it possible to design manipulation systems in which feasibility is not an implicit consequence of the chosen algorithm, but an explicit object of representation, reuse, amortization, and discovery.
...
% \header{Constraint knowledge as a design object}
% The two-paradigm landscape and the dynamic gap it produces share a common root: both paradigms treat the representation of constraint knowledge as an incidental consequence of the chosen algorithm rather than as a primary design decision.
% This thesis takes a different view.
% It asks not only whether a robot should reason about dynamic constraints, but where that reasoning should reside---evaluated online at each planning query, amortized into reusable precomputed structures, or absorbed into learned generative models.
% Each answer entails a different set of tradeoffs, and making that choice deliberately is the organizing principle of the work that follows.
